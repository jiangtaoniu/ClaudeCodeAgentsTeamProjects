...
**本项目在 WSL 中的路径**: `/mnt/f/ClaudeCodeAgentsTeamProjects`
...
项目中包含完整的 WSL 环境搭建脚本，位于 `wsl-setup/` 目录，需按顺序执行:

| 顺序 | 脚本 | 用途 | 状态 |
|------|------|------|------|
| 0 | `00-setup-sudo.sh` | sudo 免密配置 | 待确认 |
| 1 | `01-proxy-and-locale.sh` | 代理配置 + Locale 设置 | ⚠️ 已执行但端口需更新为 7897 |
| 2 | `02-system-tools.sh` | 系统工具安装 | 待执行 |
| 3 | `03-ros2-humble.sh` | ROS2 Humble 安装 | 待执行 |
| 4 | `04-cuda-toolkit.sh` | CUDA Toolkit 安装 | 待执行 |
| 5 | `05-miniconda.sh` | Miniconda + lidar3d 环境 | 待执行 |
| 6 | `06-openpcdet.sh` | OpenPCDet 安装 | 待执行 |
| 7 | `07-onnx.sh` | ONNX 运行时安装 | 待执行 |
| 8 | `08-final-verify.sh` | 最终验证 | 待执行 |
...
# 访问 Windows 文件
cd /mnt/f/ClaudeCodeAgentsTeamProjects
...
6. **lidar3d 环境**: 计划创建的 Conda 环境，用于激光雷达 3D 点云处理，包含 PyTorch (cu124)、OpenPCDet、Open3D 等。
7. **DNS 隧道**: `dnsTunneling=true` 确保 WSL 的 DNS 请求通过 Windows DNS 隧道解析，提高稳定性。
